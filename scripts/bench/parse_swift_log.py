#!/usr/bin/env python3
"""Parse SWIFT Megatron training log and output JSONL with per-iteration metrics.

Usage: python parse_swift_log.py <logfile> [--output <jsonl_file>]

Parses SWIFT Megatron training log and outputs JSONL with fields:
  {"iteration": int, "step_time_ms": float, "loss": float, "lr": float,
   "memory_gb": float, "tflops_per_gpu": float}

Extracts TFLOPS from SWIFT's native "throughput per GPU (TFLOP/s/GPU)" logging.
Skips first 5 warmup iterations for summary.
"""
...
def parse_log(logfile: str) -> list[dict]:
    """Parse a SWIFT Megatron training log file.

    SWIFT uses the same Megatron log format (from base.py training loop):
    [datetime] iteration <N>/<total> | consumed samples: ... |
    elapsed time per iteration (ms): <float> | throughput per GPU (TFLOP/s/GPU): <float> |
    learning rate: <float> | global batch size: <int> | loss: <float> | ...
    """
    entries = []

    # Same patterns as Bridge (both use Megatron logging)
    ...
    # SWIFT inline memory: "memory(GiB): 43.73"
    inline_mem_pat = re.compile(r"memory\(GiB\):\s+([\d.]+)")
    ...
    with open(logfile) as f:
        for line in f:
            ...
            if iter_match:
                ...
                inline_mem_match = inline_mem_pat.search(line)
                ...
                if inline_mem_match:
                    entry["memory_gb"] = float(inline_mem_match.group(1))
    ...
    # Also try SWIFT's JSONL log if available
    # SWIFT writes to {save_dir}/logging.jsonl
    # We can parse that too for throughput data
    return entries
...
def main():
    parser = argparse.ArgumentParser(description="Parse SWIFT Megatron training log")
